...
const $ = new Env('宝石消消乐');
let status;
status = (status = ($.getval("bsxxlstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
const bsxxlhdArr = [],bsxxlbodyArr = [],bsxxlcount = ''
let bsxxlhd = $.getdata('bsxxlhd')
let bsxxlbody = $.getdata('bsxxlbody')
...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    bsxxlhdArr.push($.getdata('bsxxlhd'))
    bsxxlbodyArr.push($.getdata('bsxxlbody'))
    let bsxxlcount = ($.getval('bsxxlcount') || '1');
  for (let i = 2; i <= bsxxlcount; i++) {
    bsxxlhdArr.push($.getdata(`bsxxlhd${i}`))
    bsxxlbodyArr.push($.getdata(`bsxxlbody${i}`))
  }
    console.log(`------------- 共${bsxxlhdArr.length}个账号-------------\n`)
      for (let i = 0; i < bsxxlhdArr.length; i++) {
        if (bsxxlhdArr[i]) {
          bsxxlhd = bsxxlhdArr[i];
          bsxxlbody = bsxxlbodyArr[i];
          $.index = i + 1;
          console.log(`\n开始【宝石消消乐${$.index}】`)
...
function bsxxlck() {
  ...
  const bsxxlhd = JSON.stringify($request.headers)
        if(bsxxlhd)    $.setdata(bsxxlhd,`bsxxlhd${status}`)
$.log(bsxxlhd)
const bsxxlbody = $request.body
        if(bsxxlbody)    $.setdata(bsxxlbody,`bsxxlbody${status}`)
$.log(bsxxlbody)
   $.msg($.name,"",'宝石消消乐'+`${status}` +'红包数据获取成功！')
  }
}
