...
const dtwzhdArr = [],dtwzbodyArr = [],dtwzspbodyArr = [],dtwzcount = ''
let dtwzhd = $.getdata('dtwzhd')
let dtwzbody = $.getdata('dtwzbody')
let dtwzspbody = $.getdata('dtwzspbody')
const sphd = {
'Accept' : `*/*`,
...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    dtwzhdArr.push($.getdata('dtwzhd'))
    dtwzbodyArr.push($.getdata('dtwzbody'))
    dtwzspbodyArr.push($.getdata('dtwzspbody'))
    let dtwzcount = ($.getval('dtwzcount') || '1');
  for (let i = 2; i <= dtwzcount; i++) {
    dtwzhdArr.push($.getdata(`dtwzhd${i}`))
    dtwzbodyArr.push($.getdata(`dtwzbody${i}`))
    dtwzspbodyArr.push($.getdata(`dtwzspbody${i}`))
  }
    console.log(`------------- 共${dtwzhdArr.length}个账号-------------\n`)
      for (let i = 0; i < dtwzhdArr.length; i++) {
        if (dtwzhdArr[i]) {
          dtwzhd = dtwzhdArr[i];
          dtwzbody = dtwzbodyArr[i];
          dtwzspbody = dtwzspbodyArr[i];
          $.index = i + 1;
          console.log(`\n开始【答题王者${$.index}】`)
...
function dtwzck() {
  ...
  const dtwzhd = JSON.stringify($request.headers)
        if(dtwzhd)    $.setdata(dtwzhd,`dtwzhd${status}`)
$.log(dtwzhd)
const dtwzbody = $request.body
        if(dtwzbody)    $.setdata(dtwzbody,`dtwzbody${status}`)
$.log(dtwzbody)
   $.msg($.name,"",'答题王者'+`${status}` +'红包数据获取成功！')
  }else if ($request.url.indexOf("reward_video/reward/") > -1) {
 
const dtwzspbody = $request.body
        if(dtwzspbody)    $.setdata(dtwzspbody,`dtwzspbody${status}`)
$.log(dtwzspbody)
   $.msg($.name,"",'答题王者'+`${status}` +'视频数据获取成功！')
  }
}

//红包
function dtwzhb(timeout = 0) {
  return new Promise((resolve) => {

let url = {
        url : 'https://r1.nullpointerexception.cn/redpacket//f/ad/divident',
        headers : JSON.parse(dtwzhd),
        body : dtwzbody,
}
