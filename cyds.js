...
const $ = new Env('成语大师');
let status;
status = (status = ($.getval("cydsstatus") || "1") ) > 1 ? `${status}` : ""; // 账号扩展字符
const cydshdArr = [],cydsbodyArr = [],cydsspbodyArr = [],cydscount = ''
let cydshd = $.getdata('cydshd')
let cydsbody = $.getdata('cydsbody')
let cydsspbody = $.getdata('cydsspbody')
const sphd = {
'Accept' : `*/*`,
...
!(async () => {
  if (typeof $request !== "undefined") {
    ...
    cydshdArr.push($.getdata('cydshd'))
    cydsbodyArr.push($.getdata('cydsbody'))
    cydsspbodyArr.push($.getdata('cydsspbody'))
    let cydscount = ($.getval('cydscount') || '1');
  for (let i = 2; i <= cydscount; i++) {
    cydshdArr.push($.getdata(`cydshd${i}`))
    cydsbodyArr.push($.getdata(`cydsbody${i}`))
    cydsspbodyArr.push($.getdata(`cydsspbody${i}`))
  }
    console.log(`------------- 共${cydshdArr.length}个账号-------------\n`)
      for (let i = 0; i < cydshdArr.length; i++) {
        if (cydshdArr[i]) {
          cydshd = cydshdArr[i];
          cydsbody = cydsbodyArr[i];
          cydsspbody = cydsspbodyArr[i];
          $.index = i + 1;
          console.log(`\n开始【成语大师${$.index}】`)
...
function cydsck() {
  ...
  const cydshd = JSON.stringify($request.headers)
        if(cydshd)    $.setdata(cydshd,`cydshd${status}`)
$.log(cydshd)
const cydsbody = $request.body
        if(cydsbody)    $.setdata(cydsbody,`cydsbody${status}`)
$.log(cydsbody)
   $.msg($.name,"",'成语大师'+`${status}` +'红包数据获取成功！')
  }else if ($request.url.indexOf("reward_video/reward/") > -1) {
 
const cydsspbody = $request.body
        if(cydsspbody)    $.setdata(cydsspbody,`cydsspbody${status}`)
$.log(cydsspbody)
   $.msg($.name,"",'成语大师'+`${status}` +'视频数据获取成功！')
  }
}

//红包
function cydshb(timeout = 0) {
  return new Promise((resolve) => {

let url = {
        url : 'https://r1.nullpointerexception.cn/redpacket//f/ad/divident',
        headers : JSON.parse(cydshd),
        body : cydsbody,
}
