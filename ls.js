...
保存到相册微信扫码注册后下载
收益：
一天两块多，10元提现，提现需一元实名认证，身份证写同名的，不需要人脸 注：工作日才能提现，单笔提现超过99元需要身份证照片，所以10块提应该可以
其他任务暂时不写，写了收益太高，怕黑，暂时不碰，稳定的话再说
请更新脚本和boxjs。
本脚本以学习为主
获取数据： 进入软件，点击我的，下拉刷新获取数据，或者点击积分获得数据，获取不到退出软件重新进入再试试
...
来闪
青龙环境抓取链接http://app.lshan8.com/ls/flash/uc/userasset/getUserAssetProperty
随便哪一个f-token都行
环境配置(@隔开，json格式)
export lsftoken='抓取的ftoken1@抓取的ftoken2'
圈X配置如下，其他自行测试
运行一次即可
[task_local]
#来闪
0,30 8 * * * https://raw.githubusercontent.com/YaphetS0903/JStest/main/ls.js, tag=来闪, enabled=true
[rewrite_local]
#来闪
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!$.isNode()) {
            ...
            console.log(`共${lsftokenArr.length}个cookie`)
    ...
    return new Promise((resolve) => {
        ...
        $.post(url, async (err, resp, data) => {
            try {
                ...
                if (result.code == 0) {
                    console.log(`【看视频】：${result.message}【获得积分】：${result.data}\n`)
                } else {
    ...
    return new Promise((resolve) => {
        ...
        $.get(url, async (err, resp, data) => {
            try {
                ...
                if (result.code == 0) {
                    console.log(`【查询签到信息】：${result.message}\n`)
                    await $.wait(5000)
                    await lssign()
                    console.log(`【当前总积分】：${result.data.integral}\n`)
                } else {
                    console.log(`【看视频失败】：${result.message}\n`)
